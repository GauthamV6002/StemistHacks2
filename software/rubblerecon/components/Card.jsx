...
const Card = ({ x, y, wasMoving, maxFunc, minFunc, width, deleteFunc, maximized, index, name, connection }) => {

  const [moving, setMoving] = useState(wasMoving);
  let mapX = useRef(x);
  let mapY = useRef(y);
  const mapRef = useRef(null);

  return (
    <CardShell shellWidth={width}>
      <div style={{maxWidth: (width - 4) + "rem"}} className="bg-gray-600 text-orange-400 m-2 rounded">
        ...
        <div className="p-2">
          <MapCanvas moving={moving} width={width} x={mapX} y={mapY} />
        </div>
      ...
      <div className="flex flex-col items-center grow">
        ...
        }} className="rounded-xl hover:bg-red-100 bg-gray-600 w-8 h-8 mr-2 mt-2 pl-0.5 flex items-center justify-center">
        ...
        <div onClick={() => {
          setMoving(!moving);
        }} className="rounded-xl hover:bg-orange-100 bg-gray-600 w-8 h-8 mr-2 mt-2 pl-0.5 flex items-center justify-center">
          {moving ?
        ...
        {maximized ? 
          <div onClick={() => {
            minFunc(index, mapX.current, mapY.current, moving);
          }} className="rounded-xl hover:bg-orange-100 bg-gray-600 w-8 h-8 mr-2 mt-2 pl-0.5 flex items-center justify-center">
        ...
        :
          <div onClick={() => {
            maxFunc(index, mapX.current, mapY.current, moving);
          }} className="rounded-xl hover:bg-orange-100 bg-gray-600 w-8 h-8 mr-2 mt-2 pl-0.5 flex items-center justify-center">
